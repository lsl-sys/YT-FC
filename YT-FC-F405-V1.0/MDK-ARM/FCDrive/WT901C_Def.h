#ifndef __WT901C_DEF_H
#define __WT901C_DEF_H
...
typedef struct
{
    uint8_t sumcrc;      // 校验和（取低8位）
                         // 计算公式: sum = (0x55+TYPE+DATA1L+DATA1H+DATA2L+DATA2H+DATA3L+DATA3H+DATA4L+DATA4H) & 0xff

    /** 加速度数据原始字节 */
    uint8_t AxL;         // X轴加速度低字节
    uint8_t AxH;         // X轴加速度高字节
                         // X轴加速度 = ((AxH<<8)|AxL)/32768*16g (g为重力加速度，取值9.8m/s²)
    uint8_t AyL;         // Y轴加速度低字节
    uint8_t AyH;         // Y轴加速度高字节
                         // Y轴加速度 = ((AyH<<8)|AyL)/32768*16g
    uint8_t AzL;         // Z轴加速度低字节
    uint8_t AzH;         // Z轴加速度高字节
                         // Z轴加速度 = ((AzH<<8)|AzL)/32768*16g
    uint8_t ATempL;      // 加速度传感器温度低字节
    uint8_t ATempH;      // 加速度传感器温度高字节

    /** 角速度数据原始字节 */
    uint8_t WxL;         // X轴角速度低字节
    uint8_t WxH;         // X轴角速度高字节
                         // X轴角速度 = ((WxH<<8)|WxL)/32768*2000°/s
    uint8_t WyL;         // Y轴角速度低字节
    uint8_t WyH;         // Y轴角速度高字节
                         // Y轴角速度 = ((WyH<<8)|WyL)/32768*2000°/s
    uint8_t WzL;         // Z轴角速度低字节
    uint8_t WzH;         // Z轴角速度高字节
                         // Z轴角速度 = ((WzH<<8)|WzL)/32768*2000°/s
    uint8_t VolL;        // 电压低字节
    uint8_t VolH;        // 电压高字节

    /** 角度数据原始字节 */
    uint8_t RollL;       // 横滚角低字节
    uint8_t RollH;       // 横滚角高字节
                         // 横滚角 = ((RollH<<8)|RollL)/32768*180°
    uint8_t PitchL;      // 俯仰角低字节
    uint8_t PitchH;      // 俯仰角高字节
                         // 俯仰角 = ((PitchH<<8)|PitchL)/32768*180°
    uint8_t YawL;        // 偏航角低字节
    uint8_t YawH;        // 偏航角高字节
                         // 偏航角 = ((YawH<<8)|YawL)/32768*180°
    uint8_t VersionL;    // 版本号低字节
    uint8_t VersionH;    // 版本号高字节
                         // 版本号 = (VH<<8)|VL
    ...
    uint8_t HzH;         // Z轴磁场高字节
                         // Z轴磁场 = ((HzH<<8)|HzL)
    uint8_t HTempL;      // 磁场传感器温度低字节
    uint8_t HTempH;      // 磁场传感器温度高字节
                         // 温度 = ((TH<<8)|TL) / 100 °C
}wt901c_raw_data;
...
#define PRESSENSOR 	0x73/** author : lsl-sys*/
